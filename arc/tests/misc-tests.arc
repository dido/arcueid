...
(register-test '(suite "miscellaneous tests"
  ...
  (suite "watch out for dangerous optimisations"
     ...
     ;; The list generated by the below functions is the following:
     ;; (= mylist '(a x x b c))
     ;; (= mylist (cons mylist mylist))
     ;; It is thus a self-referential structure completely different
     ;; from '((a x x b c) a x x b c).
     ;; Arcueid's cycle detection algorithms notice this, while reference
     ;; Arc's naive iso implementation does not.
