...
value __arc_bignum_fixnum(arc *c, value n)
{
...
static value mul_bignum(arc *c, value v1, value v2)
{
  ...
  prod = __arc_bignum_fixnum(c, prod);
  return(prod);
}
...
static value sub_bignum(arc *c, value v1, value v2)
{
  ...
  diff = __arc_bignum_fixnum(c, diff);
  return(diff);
}
...
static AFFDEF(rational_xcoerce)
{
  ...
  if (FIX2INT(AV(stype)) == T_FIXNUM || FIX2INT(AV(stype)) == T_BIGNUM) {
    ...
    if (drem >  0.5)
      ...
      mpz_add_ui(REPBNUM(bignum), REPBNUM(bignum), 1);
    ARETURN(__arc_bignum_fixnum(c, bignum));
  }
  ...
  if (FIX2INT(AV(stype)) == T_CONS) {
    ...
    num = __arc_bignum_fixnum(c, num);
    den = __arc_bignum_fixnum(c, den);
...
value __arc_rational_int(arc *c, value q)
{
  value val;
...
static value mul_rational(arc *c, value v1, value v2)
{
  ...
  prod = __arc_rational_int(c, prod);
  return(prod);
}
...
static value div_rational(arc *c, value v1, value v2)
{
  ...
  quot = __arc_rational_int(c, quot);
  return(quot);
}
...
static value sub_rational(arc *c, value v1, value v2)
{
  ...
  diff = __arc_rational_int(c, diff);
  return(diff);
}
...
#define TYPE_CASES(func, arg1, arg2) do {				\
    ...
    coerce1 = __arc_coercefn(c, arg1);					\
    coerce2 = __arc_coercefn(c, arg2);					\
...
value __arc_idiv2(arc *c, value arg1, value arg2)
{
  ...
  if (TYPE(arg1) == T_FIXNUM && TYPE(arg2) == T_FIXNUM) {
    ...
    return(__arc_bignum_fixnum(c, quot));
  }
#endif
...
value __arc_mod2(arc *c, value arg1, value arg2)
{
  ...
  if (TYPE(arg1) == T_FIXNUM && TYPE(arg2) == T_FIXNUM) {
    ...
    return(__arc_bignum_fixnum(c, modulus));
  }
#endif
...
value (*__arc_coercefn(arc *c, value val))(arc *, value, enum arc_types)
{
...
value __arc_str2flo(arc *c, value obj, value b, int strptr, int limit)
{
  ...
  coercer = __arc_coercefn(c, b);
  b = coercer(c, b, T_FLONUM);
  coercer = __arc_coercefn(c, exponent);
