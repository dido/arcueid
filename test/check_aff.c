...
START_TEST(test_aff_simple)
{
  ...
  thr = arc_mkthread(c);
  SVALR(thr, arc_mkaff(c, simple_aff, arc_mkstringc(c, "simple_aff")));
  TARGC(thr) = 0;
...
START_TEST(test_aff_subtractor)
{
  ...
  thr = arc_mkthread(c);
  SVALR(thr, arc_mkaff(c, subtractor, arc_mkstringc(c, "subtractor")));
  CPUSH(thr, INT2FIX(3));
  CPUSH(thr, INT2FIX(2));
  TARGC(thr) = 2;
  __arc_thr_trampoline(c, thr, TR_FNAPP);
...
START_TEST(test_aff_doubler)
{
  ...
  thr = arc_mkthread(c);
  SVALR(thr, arc_mkaff(c, doubler, arc_mkstringc(c, "doubler")));
  CPUSH(thr, INT2FIX(5));
  CPUSH(thr, INT2FIX(2));
  TARGC(thr) = 2;
  __arc_thr_trampoline(c, thr, TR_FNAPP);
