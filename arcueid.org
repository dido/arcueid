...
** TODO Runtime Built-In Functions [9/19]
   ...
   - [-] Evaluation [3/4]
     - [ ] eval
	   ...
	   expression to be evaluated.  There should be a special
	   entry point that compiles a sexpr as though it were
	   enclosed in a (fn () ...) form.  The output of the compiler
   ...
   - [X] Basic I/O primitives (src/io.c) [3/3]
	 ...
	 - [X] inside
       - [X] Seeking / telling [2/2]
         ...
         - [X] seek
         - [X] tell
   - [-] Compiler Support Functions [1/6]
	 These functions are expected by the Arcueid compiler.  May
	 also be used by alternate compilers targeting Arcueid
	 bytecode.
     - [X] cgenctx
     - [ ] cgen
     - [ ] cptr
     ...
     - [ ] cliteral
     - [ ] ctx2code
   - [ ] Additional I/O functions (src/io.c) [0/6]
...
** TODO Macros
   Macros can probably be handled entirely by the compiler.  After
   all, the compiler needs to have access to macro definitions.  The 
...
** Macro expansion
   ...
   - Macros may only be normal functions.  C functions cannot be used
     as macros yet.
